...
This is a standard Boto3 ``credentials`` file. You can read more about it on the `Boto3 docs <http://boto3.readthedocs.io/en/latest/guide/configuration.html>`_. The important part is that each account/environment that Foremast is managing has a distinct section in ``credentials``.
