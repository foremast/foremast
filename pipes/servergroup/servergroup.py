...
class SpinnakerServerGroup:
    ...
    def get_template(self):
        """ Gets the jinja template and renders with dictionary
        Returns: Rendered Template in json format """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_capacity",
                        help="The maximum amount of instances in a server group",
                        type=int,
                        default=1)
    parser.add_argument("--desired_capacity",
                        help="The desired amount of instances in a server group",
                        type=int,
                        default=1)
    parser.add_argument("--iamrole",
                        help="The iamrole to associate with the server group",
                        default='app_default_profile')
    parser.add_argument("--keypair",
                        help="The keypair for accessing instances in the server group")
    parser.add_argument("--subnet_type",
                        help="The subnet type to associate with security group",
                        default='internal')
    parser.add_argument("--securitygroups",
                        help="The security groups instances in the server group",
                        nargs='*',
                        default=["sg_offices", "sg_apps"])
    parser.add_argument("--elbs",
                        help="The ELBs to associate with the server group",
                        nargs='*',
                        default=[])
    parser.add_argument("--ami_id",
                        help="The ami for instances in the server group",
                        required=True)
    parser.add_argument("--instancetype",
                        help="The instance type to use",
                        default='t2.medium')
    parser.add_argument("--b64_userdata",
                        help="The userdata for instances in server group",
                        default='')
